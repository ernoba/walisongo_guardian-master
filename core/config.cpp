...
namespace Config {
    ...
    // 02 Februari 2027 00:00:00 GMT+0
    time_t EXPIRY_DATE = 1801526400;

    // 4. Variabel Sistem Lainnya
    const std::wstring INSTALL_LICENSE = L"ASD";
    ...
    // Daftar blacklist
    const std::vector<std::string> BLACKLIST = {"facebook", "instagram", "youtube", "tiktok", "whatsapp", "telegram", "pornhub", "yandex", "login", "akun", "slot"};
    
    // Update daftar SSID WiFi yang dilarang (Termasuk target baru)
    const std::vector<std::string> WIFI_BLACKLIST = {"Songo-net DOKUMENTASI", "Songo Net-DJANA"};
}